...
@dataclass
class Config:
    """Configuration for trace_collector"""
    enabled: bool = True
    timeout: int = 30
...
def create_trace_collector(config: Optional[Config] = None) -> TraceCollector:
    """Factory function to create trace_collector instance"""
    if config is None:
        config = Config()
