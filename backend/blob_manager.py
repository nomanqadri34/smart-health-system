...
@dataclass
class Config:
    """Configuration for blob_manager"""
    enabled: bool = True
    timeout: int = 30
...
def create_blob_manager(config: Optional[Config] = None) -> BlobManager:
    """Factory function to create blob_manager instance"""
    if config is None:
        config = Config()
    return BlobManager(config)
