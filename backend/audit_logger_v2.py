...
@dataclass
class Config:
    """Configuration for audit_logger_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_audit_logger_v2(config: Optional[Config] = None) -> AuditLoggerV2:
    """Factory function to create audit_logger_v2 instance"""
    if config is None:
        config = Config()
    return AuditLoggerV2(config)
