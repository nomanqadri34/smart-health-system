...
@dataclass
class Config:
    """Configuration for alert_correlator"""
    enabled: bool = True
    timeout: int = 30
...
def create_alert_correlator(config: Optional[Config] = None) -> AlertCorrelator:
    """Factory function to create alert_correlator instance"""
    if config is None:
        config = Config()
