...
@dataclass
class Config:
    """Configuration for model_explainer"""
    enabled: bool = True
    timeout: int = 30
...
def create_model_explainer(config: Optional[Config] = None) -> ModelExplainer:
    """Factory function to create model_explainer instance"""
    if config is None:
        config = Config()
