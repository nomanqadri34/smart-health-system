...
@dataclass
class Config:
    """Configuration for quota_enforcer"""
    enabled: bool = True
    timeout: int = 30
...
def create_quota_enforcer(config: Optional[Config] = None) -> QuotaEnforcer:
    """Factory function to create quota_enforcer instance"""
    if config is None:
        config = Config()
    return QuotaEnforcer(config)
