...
@dataclass
class Config:
    """Configuration for compression_manager_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_compression_manager_v2(config: Optional[Config] = None) -> CompressionManagerV2:
    """Factory function to create compression_manager_v2 instance"""
    if config is None:
        config = Config()
    return CompressionManagerV2(config)
