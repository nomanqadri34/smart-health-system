...
@dataclass
class Config:
    """Configuration for stream_processor_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_stream_processor_v2(config: Optional[Config] = None) -> StreamProcessorV2:
    """Factory function to create stream_processor_v2 instance"""
    if config is None:
        config = Config()
    return StreamProcessorV2(config)
