...
@dataclass
class Config:
    """Configuration for firewall_manager"""
    enabled: bool = True
    timeout: int = 30
...
def create_firewall_manager(config: Optional[Config] = None) -> FirewallManager:
    """Factory function to create firewall_manager instance"""
    if config is None:
        config = Config()
    return FirewallManager(config)
