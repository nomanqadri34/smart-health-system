...
@dataclass
class Config:
    """Configuration for hyperparameter_tuner"""
    enabled: bool = True
    timeout: int = 30
...
def create_hyperparameter_tuner(config: Optional[Config] = None) -> HyperparameterTuner:
    """Factory function to create hyperparameter_tuner instance"""
    if config is None:
        config = Config()
