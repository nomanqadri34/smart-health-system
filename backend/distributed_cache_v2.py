...
@dataclass
class Config:
    """Configuration for distributed_cache_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_distributed_cache_v2(config: Optional[Config] = None) -> DistributedCacheV2:
    """Factory function to create distributed_cache_v2 instance"""
    if config is None:
        config = Config()
    return DistributedCacheV2(config)
