...
@dataclass
class Config:
    """Configuration for aggregation_engine"""
    enabled: bool = True
    timeout: int = 30
...
def create_aggregation_engine(config: Optional[Config] = None) -> AggregationEngine:
    """Factory function to create aggregation_engine instance"""
    if config is None:
        config = Config()
