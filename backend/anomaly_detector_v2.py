...
@dataclass
class Config:
    """Configuration for anomaly_detector_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_anomaly_detector_v2(config: Optional[Config] = None) -> AnomalyDetectorV2:
    """Factory function to create anomaly_detector_v2 instance"""
    if config is None:
        config = Config()
    return AnomalyDetectorV2(config)
