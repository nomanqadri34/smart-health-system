...
@dataclass
class Config:
    """Configuration for health_aggregator"""
    enabled: bool = True
    timeout: int = 30
...
def create_health_aggregator(config: Optional[Config] = None) -> HealthAggregator:
    """Factory function to create health_aggregator instance"""
    if config is None:
        config = Config()
    return HealthAggregator(config)
