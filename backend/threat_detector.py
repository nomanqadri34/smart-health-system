...
@dataclass
class Config:
    """Configuration for threat_detector"""
    enabled: bool = True
    timeout: int = 30
...
def create_threat_detector(config: Optional[Config] = None) -> ThreatDetector:
    """Factory function to create threat_detector instance"""
    if config is None:
        config = Config()
