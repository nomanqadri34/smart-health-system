...
@dataclass
class Config:
    """Configuration for retry_manager_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_retry_manager_v2(config: Optional[Config] = None) -> RetryManagerV2:
    """Factory function to create retry_manager_v2 instance"""
    if config is None:
        config = Config()
    return RetryManagerV2(config)
