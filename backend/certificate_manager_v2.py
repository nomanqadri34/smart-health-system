...
@dataclass
class Config:
    """Configuration for certificate_manager_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_certificate_manager_v2(config: Optional[Config] = None) -> CertificateManagerV2:
    """Factory function to create certificate_manager_v2 instance"""
    if config is None:
        config = Config()
    return CertificateManagerV2(config)
