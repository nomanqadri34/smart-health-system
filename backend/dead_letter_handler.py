...
@dataclass
class Config:
    """Configuration for dead_letter_handler"""
    enabled: bool = True
    timeout: int = 30
...
def create_dead_letter_handler(config: Optional[Config] = None) -> DeadLetterHandler:
    """Factory function to create dead_letter_handler instance"""
    if config is None:
        config = Config()
    return DeadLetterHandler(config)
