...
@dataclass
class Config:
    """Configuration for metrics_collector_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_metrics_collector_v2(config: Optional[Config] = None) -> MetricsCollectorV2:
    """Factory function to create metrics_collector_v2 instance"""
    if config is None:
        config = Config()
    return MetricsCollectorV2(config)
