...
@dataclass
class Config:
    """Configuration for inference_engine"""
    enabled: bool = True
    timeout: int = 30
...
def create_inference_engine(config: Optional[Config] = None) -> InferenceEngine:
    """Factory function to create inference_engine instance"""
    if config is None:
        config = Config()
