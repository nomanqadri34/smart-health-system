...
@dataclass
class Config:
    """Configuration for rate_limiter_v3"""
    enabled: bool = True
    timeout: int = 30
...
def create_rate_limiter_v3(config: Optional[Config] = None) -> RateLimiterV3:
    """Factory function to create rate_limiter_v3 instance"""
    if config is None:
        config = Config()
    return RateLimiterV3(config)
