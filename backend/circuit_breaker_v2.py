...
@dataclass
class Config:
    """Configuration for circuit_breaker_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_circuit_breaker_v2(config: Optional[Config] = None) -> CircuitBreakerV2:
    """Factory function to create circuit_breaker_v2 instance"""
    if config is None:
        config = Config()
    return CircuitBreakerV2(config)
