...
@dataclass
class Config:
    """Configuration for jwt_manager_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_jwt_manager_v2(config: Optional[Config] = None) -> JwtManagerV2:
    """Factory function to create jwt_manager_v2 instance"""
    if config is None:
        config = Config()
    return JwtManagerV2(config)
