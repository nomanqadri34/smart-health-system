...
@dataclass
class Config:
    """Configuration for gossip_protocol"""
    enabled: bool = True
    timeout: int = 30
...
def create_gossip_protocol(config: Optional[Config] = None) -> GossipProtocol:
    """Factory function to create gossip_protocol instance"""
    if config is None:
        config = Config()
    return GossipProtocol(config)
