...
@dataclass
class Config:
    """Configuration for data_replicator"""
    enabled: bool = True
    timeout: int = 30
...
def create_data_replicator(config: Optional[Config] = None) -> DataReplicator:
    """Factory function to create data_replicator instance"""
    if config is None:
        config = Config()
    return DataReplicator(config)
