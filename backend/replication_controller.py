...
@dataclass
class Config:
    """Configuration for replication_controller"""
    enabled: bool = True
    timeout: int = 30
...
def create_replication_controller(config: Optional[Config] = None) -> ReplicationController:
    """Factory function to create replication_controller instance"""
    if config is None:
        config = Config()
    return ReplicationController(config)
