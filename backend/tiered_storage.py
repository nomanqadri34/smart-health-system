...
@dataclass
class Config:
    """Configuration for tiered_storage"""
    enabled: bool = True
    timeout: int = 30
...
def create_tiered_storage(config: Optional[Config] = None) -> TieredStorage:
    """Factory function to create tiered_storage instance"""
    if config is None:
        config = Config()
    return TieredStorage(config)
