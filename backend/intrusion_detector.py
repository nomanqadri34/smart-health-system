...
@dataclass
class Config:
    """Configuration for intrusion_detector"""
    enabled: bool = True
    timeout: int = 30
...
def create_intrusion_detector(config: Optional[Config] = None) -> IntrusionDetector:
    """Factory function to create intrusion_detector instance"""
    if config is None:
        config = Config()
    return IntrusionDetector(config)
