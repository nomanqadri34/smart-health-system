...
@dataclass
class Config:
    """Configuration for storage_optimizer"""
    enabled: bool = True
    timeout: int = 30
...
def create_storage_optimizer(config: Optional[Config] = None) -> StorageOptimizer:
    """Factory function to create storage_optimizer instance"""
    if config is None:
        config = Config()
    return StorageOptimizer(config)
