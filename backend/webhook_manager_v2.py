...
@dataclass
class Config:
    """Configuration for webhook_manager_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_webhook_manager_v2(config: Optional[Config] = None) -> WebhookManagerV2:
    """Factory function to create webhook_manager_v2 instance"""
    if config is None:
        config = Config()
    return WebhookManagerV2(config)
