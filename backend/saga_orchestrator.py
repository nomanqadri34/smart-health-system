...
@dataclass
class Config:
    """Configuration for saga_orchestrator"""
    enabled: bool = True
    timeout: int = 30
...
def create_saga_orchestrator(config: Optional[Config] = None) -> SagaOrchestrator:
    """Factory function to create saga_orchestrator instance"""
    if config is None:
        config = Config()
