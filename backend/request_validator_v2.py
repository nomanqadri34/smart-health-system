...
@dataclass
class Config:
    """Configuration for request_validator_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_request_validator_v2(config: Optional[Config] = None) -> RequestValidatorV2:
    """Factory function to create request_validator_v2 instance"""
    if config is None:
        config = Config()
    return RequestValidatorV2(config)
