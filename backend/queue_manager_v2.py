...
@dataclass
class Config:
    """Configuration for queue_manager_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_queue_manager_v2(config: Optional[Config] = None) -> QueueManagerV2:
    """Factory function to create queue_manager_v2 instance"""
    if config is None:
        config = Config()
    return QueueManagerV2(config)
