...
@dataclass
class Config:
    """Configuration for incident_manager_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_incident_manager_v2(config: Optional[Config] = None) -> IncidentManagerV2:
    """Factory function to create incident_manager_v2 instance"""
    if config is None:
        config = Config()
    return IncidentManagerV2(config)
