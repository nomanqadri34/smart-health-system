...
@dataclass
class Config:
    """Configuration for partition_manager"""
    enabled: bool = True
    timeout: int = 30
...
def create_partition_manager(config: Optional[Config] = None) -> PartitionManager:
    """Factory function to create partition_manager instance"""
    if config is None:
        config = Config()
