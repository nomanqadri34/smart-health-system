...
@dataclass
class Config:
    """Configuration for schema_validator_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_schema_validator_v2(config: Optional[Config] = None) -> SchemaValidatorV2:
    """Factory function to create schema_validator_v2 instance"""
    if config is None:
        config = Config()
    return SchemaValidatorV2(config)
