...
@dataclass
class Config:
    """Configuration for response_transformer"""
    enabled: bool = True
    timeout: int = 30
...
def create_response_transformer(config: Optional[Config] = None) -> ResponseTransformer:
    """Factory function to create response_transformer instance"""
    if config is None:
        config = Config()
