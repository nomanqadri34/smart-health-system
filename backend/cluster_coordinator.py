...
@dataclass
class Config:
    """Configuration for cluster_coordinator"""
    enabled: bool = True
    timeout: int = 30
...
def create_cluster_coordinator(config: Optional[Config] = None) -> ClusterCoordinator:
    """Factory function to create cluster_coordinator instance"""
    if config is None:
        config = Config()
