...
@dataclass
class Config:
    """Configuration for message_serializer"""
    enabled: bool = True
    timeout: int = 30
...
def create_message_serializer(config: Optional[Config] = None) -> MessageSerializer:
    """Factory function to create message_serializer instance"""
    if config is None:
        config = Config()
    return MessageSerializer(config)
