...
@dataclass
class Config:
    """Configuration for capacity_planner"""
    enabled: bool = True
    timeout: int = 30
...
def create_capacity_planner(config: Optional[Config] = None) -> CapacityPlanner:
    """Factory function to create capacity_planner instance"""
    if config is None:
        config = Config()
