...
@dataclass
class Config:
    """Configuration for data_masking_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_data_masking_v2(config: Optional[Config] = None) -> DataMaskingV2:
    """Factory function to create data_masking_v2 instance"""
    if config is None:
        config = Config()
    return DataMaskingV2(config)
