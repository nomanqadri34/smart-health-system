...
@dataclass
class Config:
    """Configuration for dashboard_generator"""
    enabled: bool = True
    timeout: int = 30
...
def create_dashboard_generator(config: Optional[Config] = None) -> DashboardGenerator:
    """Factory function to create dashboard_generator instance"""
    if config is None:
        config = Config()
