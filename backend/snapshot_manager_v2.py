...
@dataclass
class Config:
    """Configuration for snapshot_manager_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_snapshot_manager_v2(config: Optional[Config] = None) -> SnapshotManagerV2:
    """Factory function to create snapshot_manager_v2 instance"""
    if config is None:
        config = Config()
    return SnapshotManagerV2(config)
