...
@dataclass
class Config:
    """Configuration for data_labeler"""
    enabled: bool = True
    timeout: int = 30
...
def create_data_labeler(config: Optional[Config] = None) -> DataLabeler:
    """Factory function to create data_labeler instance"""
    if config is None:
        config = Config()
    return DataLabeler(config)
