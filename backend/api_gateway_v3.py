...
@dataclass
class Config:
    """Configuration for api_gateway_v3"""
    enabled: bool = True
    timeout: int = 30
...
def create_api_gateway_v3(config: Optional[Config] = None) -> ApiGatewayV3:
    """Factory function to create api_gateway_v3 instance"""
    if config is None:
        config = Config()
    return ApiGatewayV3(config)
