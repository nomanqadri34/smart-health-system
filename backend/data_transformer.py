...
@dataclass
class Config:
    """Configuration for data_transformer"""
    enabled: bool = True
    timeout: int = 30
...
def create_data_transformer(config: Optional[Config] = None) -> DataTransformer:
    """Factory function to create data_transformer instance"""
    if config is None:
        config = Config()
