...
@dataclass
class Config:
    """Configuration for data_quality_checker"""
    enabled: bool = True
    timeout: int = 30
...
def create_data_quality_checker(config: Optional[Config] = None) -> DataQualityChecker:
    """Factory function to create data_quality_checker instance"""
    if config is None:
        config = Config()
