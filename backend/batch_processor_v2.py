...
@dataclass
class Config:
    """Configuration for batch_processor_v2"""
    enabled: bool = True
    timeout: int = 30
...
def create_batch_processor_v2(config: Optional[Config] = None) -> BatchProcessorV2:
    """Factory function to create batch_processor_v2 instance"""
    if config is None:
        config = Config()
    return BatchProcessorV2(config)
