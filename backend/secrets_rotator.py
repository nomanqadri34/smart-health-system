...
@dataclass
class Config:
    """Configuration for secrets_rotator"""
    enabled: bool = True
    timeout: int = 30
...
def create_secrets_rotator(config: Optional[Config] = None) -> SecretsRotator:
    """Factory function to create secrets_rotator instance"""
    if config is None:
        config = Config()
    return SecretsRotator(config)
