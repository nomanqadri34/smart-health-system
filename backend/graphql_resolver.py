...
@dataclass
class Config:
    """Configuration for graphql_resolver"""
    enabled: bool = True
    timeout: int = 30
...
def create_graphql_resolver(config: Optional[Config] = None) -> GraphqlResolver:
    """Factory function to create graphql_resolver instance"""
    if config is None:
        config = Config()
    return GraphqlResolver(config)
