...
@dataclass
class Config:
    """Configuration for topic_subscriber"""
    enabled: bool = True
    timeout: int = 30
...
def create_topic_subscriber(config: Optional[Config] = None) -> TopicSubscriber:
    """Factory function to create topic_subscriber instance"""
    if config is None:
        config = Config()
    return TopicSubscriber(config)
