...
@dataclass
class Config:
    """Configuration for event_sourcing"""
    enabled: bool = True
    timeout: int = 30
...
def create_event_sourcing(config: Optional[Config] = None) -> EventSourcing:
    """Factory function to create event_sourcing instance"""
    if config is None:
        config = Config()
    return EventSourcing(config)
